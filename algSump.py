...
SIGMA USER CONFIGURATION (Option A - self learning)
  ...
  Sump motor load should be fairly constant for a given discharge pipe diameter and length, 
    lift height, and pump size

  Sump motor run time should be fairly constant based on float switch operation (ON-OFF float positions),
    well volume, and pump rate (motor HP, pipe diameter and length, lift height, etc.


HP USER CONFIGURATION (Option B - based on user entry of motor HP)
...
def calAlgInit(chanNames) : 
    ...
    if len(chanNames) > 1 :
        ...
        motorAlgs[chanNames[1]]["HP_ALG_ENABLE"] = 1
