...
POSSIBLE REVISIONS/ENHANCEMENTS:

GENERAL INFO

  Potential Conditions that would generate Runtime / Power Alerts (modify settings in algSump.py):
  - Frozen discharge/drain pipe (power)
  - Motor / impeller (power)
  - Motor starter capacitor (run time)
  - Water level switch stuck on (run time)
...
import algSump          # Sump pump algorithms for alerts
import pubScribe
...
chanOnThresholds = [5, 20]                     # Watts, with one entry per chanName[]
...
def readPower() :
    ...
    for chan in range(0, len(chanNames)) :
        ...
        if (power[chan] > chanOnThresholds[chan]) :
            ...
            hdr, returnStr, alertMsg = algSump.motorStats(chan, chanNames, rt, tInterval)

            timeStr = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
            messageText = "{:.2f},{},{}".format(t, timeStr, returnStr+"        ")
...
# https://docs.python.org/3/library/string.html?highlight=number%20string
#
def energyLog():
    hdr = ""
...
def displayLabels() :
    lbl = ["Time: ", "Voltage (V)     :","Amperage (A)    :","Power (W)       :",
        ...
        "Today ", "          cycles: ", "         min (s): ", "         max (s): ", "  total run time: ",  "      power (Wh): ", "",
        "Yesterday ", "          cycles: ", "         min (s): ", "         max (s): ", "  total run time: ",  "      power (Wh): ", ""]
